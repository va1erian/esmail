...
//! * receives finished outputs -- ready-to-upload pixel frames -- in
//!   [`WebView::show`], uploading the newest frame to an egui
        ...
        loop {
            match self.rx.try_recv() {
                Ok(Output::Frame(frame)) if frame.id == self.submitted_id => {
                    ...
                    self.accept_runs(frame.runs, frame.links);
...
mod tests {
    ...
    fn a_frame_carries_the_links_of_the_page_it_shows() {
        ...
        worker.render(&RenderJob { id: 1, html, width: 200.0, scale: 1.0, reset_images: true, backend: Backend::Pixbuf });
        let outputs: Vec<Output> = out_rx.try_iter().collect();
    ...
    fn a_link_shows_a_hand_and_other_text_an_i_beam() {
        for backend in BACKENDS {
            a_link_shows_a_hand_and_other_text_an_i_beam_on(backend);
        }
    }

    fn a_link_shows_a_hand_and_other_text_an_i_beam_on(backend: Backend) {
