...
//!
//! Like the [`crate::TextRunTable`], it is recorded during the draw pass, while
//! the litehtml `Document` is alive, and shipped to the UI thread with the
//! frame. Clicks and the hover cursor are then a point-in-rectangle lookup:
//! no second parse + layout per click (issue #27), and no `Document` needed.
//!
//! Coordinates are document points, the same space as the text runs.
...
impl LinkTable {
    ...
    pub fn href_at(&self, p: egui::Pos2) -> Option<&str> {
        self.links
            .iter()
            .rev()
...
fn rect_of(p: litehtml::Position) -> Option<egui::Rect> {
    (p.width > 0.0 && p.height > 0.0)
        .then(|| egui::Rect::from_min_size(egui::pos2(p.x, p.y), egui::vec2(p.width, p.height)))
}
...
fn link_rects<'a>(anchor: &Element<'a>) -> Vec<egui::Rect> {
    let mut rects: Vec<egui::Rect> = anchor.inline_boxes().into_iter().filter_map(rect_of).collect();
    if rects.is_empty() {
...
mod tests {
    ...
    fn a_block_link_covers_its_box() {
        ...
        assert_eq!(t.href_at(egui::pos2(5.0, 5.0)), Some("https://example.com/x"));
        assert_eq!(t.href_at(egui::pos2(5.0, 300.0)), None);
    }
    ...
    fn a_link_that_wraps_covers_every_line() {
        ...
        assert_eq!(t.href_at(egui::pos2(1.0, link.rects[0].center().y)), None);
    }
    ...
    fn an_image_link_covers_the_image() {
        ...
        assert_eq!(t.href_at(egui::pos2(30.0, 15.0)), Some("https://example.com/i"));
        assert_eq!(t.href_at(egui::pos2(200.0, 15.0)), None);
    }
    ...
    #[test]
    fn nested_tables_still_resolve_to_the_right_link() {
        let t = table_for(
            r#"<body style="margin:0"><table><tr><td><table><tr><td>
               <a href="https://example.com/a">alpha</a></td><td><a href="https://example.com/b">beta</a></td></tr></table></td></tr></table></body>"#,
            300.0,
        );
        let [a, b] = [&t.links[0], &t.links[1]];
        assert_eq!((a.href.as_str(), b.href.as_str()), ("https://example.com/a", "https://example.com/b"));
        assert_eq!(t.href_at(a.rects[0].center()), Some("https://example.com/a"));
        assert_eq!(t.href_at(b.rects[0].center()), Some("https://example.com/b"));
    }
